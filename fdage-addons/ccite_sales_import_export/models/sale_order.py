...
class SaleOrder(models.Model):
    ...
    def _export_csv_files(self):
        ...
        # Commerciaux
        filepath = os.path.join(export_path, 'representant.csv')
        with open(filepath, 'w', newline='') as csvfile:
            fieldnames = ['gcl_commercial', 'gcl_libelle', 'gcl_prenom', 'gcl_etablissement', 'gcl_zonecom', 'vallibre_1', 'vallibre_2',
                          'Fermé', 'gcl_email', 'zcc_telephone']
        ...
        filepath = os.path.join(export_path, 'tarif.csv')
        self._cr.execute(
        ...
        filepath = os.path.join(export_path, 'famart.csv')
        ...
        # #Produits
        filepath = os.path.join(export_path, 'article.csv')
        self._cr.execute(
            "SELECT p.default_code, p.name, CASE WHEN COALESCE(c2.code, '') = '0' THEN '0' ELSE c1.code END as code, p.active FROM product_template p "
        ...
        # # Facture
        filepath = os.path.join(export_path, 'histoentete.csv')
        self._cr.execute(
            "SELECT account_move.name,account_move.invoice_date,p1.ref as ref_tiers,p2.ref as ref_representant,account_move.amount_untaxed,account_move.amount_total FROM account_move "
        ...
        with open(filepath, 'w', newline='') as csvfile:
            fieldnames = ['gp_naturepieceg', 'gp_souche', 'gp_numero', 'gp_datepiece', 'gp_tiers', 'gp_representant', 'gp_totalht',
                          'gp_totalttc']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter=';')
            writer.writeheader()
            for record in records:
        ...
        # Lignes de Facture
        filepath = os.path.join(export_path, 'histoligne.csv')
        ...
        with open(filepath, 'w', newline='') as csvfile:
            fieldnames = ['gl_naturepieceg', 'gl_souche', 'gl_numero', 'gl_numligne', 'gl_datepiece', 'gl_tiers', 'gl_representant',
                          'gl_codearticle', 'gl_puht', 'gl_puttc', 'gl_qtefact', 'gl_remiseligne', 'gl_montantht', 'gl_montantttc']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter=';')
            writer.writeheader()
            cpt = 0
            saved_record_name = ''
        ...
        records = self._cr.dictfetchall()
        with open(filepath, 'w', newline='') as csvfile:
            fieldnames = ['t_tiers', 't_libelle', 't_adresse1', 't_adresse2', 't_adresse3', 't_codepostal', 't_ville', 't_telephone',
                          't_telephone2', 't_telex', 't_fax', 't_email', 't_siret', 't_commentaire', 't_zonecom', 't_representant',
                          't_etatrisque', 'Fermé', 'pays']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter=';')
            writer.writeheader()
            for record in records:
        ...
        # Adresses de clients
        filepath = os.path.join(export_path, 'adresse.csv')
        with open(filepath, 'w', newline='') as csvfile:

            # Write header
            fieldnames = ['adr_num', 't_tiers', 't_libelle', 't_adresse1', 't_adresse2', 't_adresse3', 't_codepostal', 't_ville',
                          't_telephone', 'adr_livr', 'adr_fact', 'adr_livrdef', 'adr_factdef']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter=';')
            writer.writeheader()
                    ...
                    for row in csv_reader:
                        if line_count > 0:
                            ...
                            if len(res_partner) > 1:
                                ...
                                _logger.warning('Date : %s  Fichier : %s double res partner trouvé avec comme ref %s ligne %s' % (
                            ...
                            if not res_partner:
                                ...
                                _logger.error('Date : %s  Fichier : %s pas de correspondance pour le res partner suivant %s ligne %s' % (
                                    date_error_start, file, row[8], line_count))
                            ...
                            if saylor_id:
                                if len(saylor_id) > 1:
                                    _logger.warning('Date : %s  Fichier : %s double vendeur trouvé avec comme ref : %s ligne %s' % (
                            ...
                            if not saylor_id:
                                _logger.error('Date : %s  Fichier : %s pas de correspondance pour le vendeur : %s ligne %s' % (
                                    date_error_start, file, row[21], line_count))
                            ...
                            if row[3] and self.env['sale.order'].search([('ep_number_swing', '=', row[3])]):
                                error_in_line_file = True
                                _logger.error('Date : %s  Fichier : %s il éxiste déjà un devis avec comme référence : %s ligne %s' % (
                            ...
                            if not error_in_line_file:
                                swing_ref = row[12]
                                commitment_date = datetime.strftime(datetime.strptime(row[11], "%d/%m/%Y"), '%Y-%m-%d') if row[
                                    11] else False
                                ...
                                new_test = self.env['sale.order'].with_context(mail_notrack=True, mail_create_nolog=True).create(
                                    sale_record_values)
                        line_count += 1
                    ...
                    for row in csv_reader:
                        if line_count > 0:
                            ...
                            if not sale_order:
                                error_in_line_file = True
                                fichier.write(
                                    'Date : %s  Fichier : %s pas de correspondance pour le sale order avec comme numéro de référence : %s ligne %s \n' % (
                            ...
                            if not product_id:
                                ...
                                fichier.write(
                                    'Date : %s  Fichier : %s pas de correspondance pour le produit avec comme numéro de référence : %s ligne %s \n' % (
                            ...
                            if not error_in_line_file:
                                ...
                                self.env['sale.order.line'].with_context(mail_notrack=True, mail_create_nolog=True).create(
                                    sale_order_line_values)
                        ...
                        if line_count > 0:
                            ...
                            if not delivery_record:
                                ...
                                if sale_order:
                                    ...
                                    self.generate_note('sale.order', sale_order.id, sale_order.name, error_message)
                                fichier.write(
                                    'Date : %s  Fichier : %s pas de correspondance pour le frais de port avec comme numéro de référence : %s ligne %s\n' % (
                                        date_error_start, file, row[4], line_count))
                                _logger.error(
                                    'Date : %s  Fichier : %s pas de correspondance pour le frais de port avec comme numéro de référence : %s ligne %s' % (
                                        date_error_start, file, row[4], line_count))
                                ...
                                if delivery_values.get('success'):
                                    ...
                                    if sale_order:
                                        self.generate_note('sale.order', sale_order.id, sale_order.name, error_message)
                                    error_msg = 'Date : %s  Fichier : %s %s pour le numéro de référence : %s' % (
                                        date_error_start, file, error_message, ep_number_swing)
